function Get-TargetResource
{
    ...
    try
    {
        ...
        $getValue = $null
        #region resource generator code
        $getValue = Get-MgPolicyAdminConsentRequestPolicy  -ErrorAction SilentlyContinue

        if ($null -eq $getValue)
        {
            Write-Verbose -Message "Could not find an Azure AD Admin Consent Request Policy with Id {$Id}"

            if (-Not [string]::IsNullOrEmpty($DisplayName))
            {
                $getValue = Get-MgPolicyAdminConsentRequestPolicy `
                    -ErrorAction SilentlyContinue | Where-Object `
                    -FilterScript { `
                        $_.DisplayName -eq "$($DisplayName)" `
                        -and $_.AdditionalProperties.'@odata.type' -eq "#microsoft.graph.AdminConsentRequestPolicy" `
                    }
            }
        }
        #endregion
        if ($null -eq $getValue)
        {
            Write-Verbose -Message "Could not find an Azure AD Admin Consent Request Policy with DisplayName {$DisplayName}"
            return $nullResult
        }
        $Id = $getValue.Id
        Write-Verbose -Message "An Azure AD Admin Consent Request Policy with Id {$Id} and DisplayName {$DisplayName} was found."

        #region resource generator code
        $complexReviewers = @()
        foreach ($currentReviewers in $getValue.reviewers)
        {
            $myReviewers = @{}
            $myReviewers.Add('Query', $currentReviewers.query)
            $myReviewers.Add('QueryRoot', $currentReviewers.queryRoot)
            $myReviewers.Add('QueryType', $currentReviewers.queryType)
            if ($myReviewers.values.Where({$null -ne $_}).count -gt 0)
            {
                $complexReviewers += $myReviewers
            }
        }
        #endregion
        ...
        $results = @{
            ...
            NotifyReviewers       = $getValue.NotifyReviewers
            RemindersEnabled      = $getValue.RemindersEnabled
            RequestDurationInDays = $getValue.RequestDurationInDays
...
function Set-TargetResource
{
    ...
    {
        Write-Verbose -Message "Creating an Azure AD Admin Consent Request Policy with DisplayName {$DisplayName}"
    ...
    {
        Write-Verbose -Message "Updating the Azure AD Admin Consent Request Policy with Id {$($currentInstance.Id)}"
...
function Test-TargetResource
{
    ...
    Write-Verbose -Message "Testing configuration of the Azure AD Admin Consent Request Policy with Id {$Id} and DisplayName {$DisplayName}"
...
function Export-TargetResource
{
    ...
    {
        #region resource generator code
        [array]$getValue = Get-MgPolicyAdminConsentRequestPolicy `
            -ErrorAction Stop
        #endregion
        ...
        foreach ($config in $getValue)
        {
            ...
            if ($null -ne $Results.Reviewers)
            {
                $complexTypeStringResult = Get-M365DSCDRGComplexTypeToString `
                    ...
                    -CIMInstanceName 'MicrosoftGraphaccessReviewReviewerScope'
